...
function assertPhase1aResolverInvariants(file: string, label: string) {
  ...
  const EDUCATION_CONSUMERS = new Set([
    ...
    // QA-R2 #5/#6: the grader resolves a diagnostic through lib/education/deca-diagnostic-bridge, and
    // Home's suggestion through lib/education/deca-home-suggestion, which reads that bridge; test
    // feedback resolves through the per-track lib/education/test-result-recommendations. Each imports
    // one module (education-registry-smoke 16b).
