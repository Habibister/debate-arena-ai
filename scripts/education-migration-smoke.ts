...
function assertPhase1aResolverInvariants(file: string, label: string) {
  ...
  assert.equal(EDUCATION_LESSONS.length, 15, "5. fifteen canonical registry entries");
  const learnerVisible = EDUCATION_LESSONS.filter((e) => e.visibility === "learner");
  assert.equal(learnerVisible.length, 15, "6a. all fifteen are learner-visible (P1-B3 added the DECA customer-relations lesson)");
  ...
  assert.equal(educationLessonsForTrack("DECA").length, 4, "6b. four DECA lessons across two courses");
  assert.deepEqual(educationLessonsForTrack("DECA").map((e) => e.id),
    ...
    "6b2. orientation, the two role-play skill owners, then the cluster-knowledge owner");
  ...
  // provably exists. These controls bind the mapping, its exactness, and its absence elsewhere.
  {
    ...
    assert.deepEqual(mapped.map((e) => e.id),
      ["debate-evidence-evaluation", "debate-answer-types", "debate-turn-mechanics", "deca-understanding-performance-indicators", "deca-justifying-your-recommendation", "deca-handling-customer-situations", "debate-clash", "debate-refutation", "debate-weighing"],
      `36. exactly nine authored lessons name a drill destination — evidence, answer-types (CTA-only), turn-mechanics (CTA-only), the two DECA role-play owners, the DECA cluster-knowledge owner, clash, refutation and weighing  [${mapped.map((e) => e.id).join(", ")}]`);
