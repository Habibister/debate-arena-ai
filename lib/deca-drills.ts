...
export const DECA_DRILL_BANK: DecaDrillQuestion[] = [
  ...
  { id: "pi-19", area: "performance-indicators", question: "A response to a listed indicator gives a clear decision, sound reasoning and a workable rollout plan, but never says how anyone would know it worked. Against a complete demonstration, it:", choices: ["Is missing a success measure, which a full demonstration includes whenever the scenario allows one", "Is complete, because measurement would belong to a different indicator", "Is incomplete because it did not restate the indicator at the end", "Is incomplete because the rollout plan should have come first"], correctAnswer: "Is missing a success measure, which a full demonstration includes whenever the scenario allows one", explanation: "A demonstration runs from decision through reasoning and implementation to how success gets checked. Leaving the check out leaves the judge no way to see the idea was thought through to the end. Where a scenario genuinely offers nothing to measure, say so rather than inventing a number." },
  ...
  { id: "pi-30", area: "performance-indicators", question: "The listed indicator is \"Explain the nature of customer retention.\" You decide to launch a win-back email to lapsed gym members. Which measure actually tells the judge whether that action worked?", choices: ["The share of lapsed members who return and are still active three months later", "The number of win-back emails the system sent", "The gym's total revenue for the year", "That you would keep an eye on it and follow up later"], correctAnswer: "The share of lapsed members who return and are still active three months later", explanation: "Retention is about members coming back and staying, so the measure has to track exactly that. Emails sent counts activity rather than results, annual revenue moves for a dozen unrelated reasons, and 'keep an eye on it' names no measure at all. A perfectly respectable business metric that does not match what the indicator asked about still leaves the judge without an answer." },
