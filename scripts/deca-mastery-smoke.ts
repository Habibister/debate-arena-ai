...
async function main() {
  ...
  assert.equal(hosaBank.MEDTERM_BANK.length, 138, "26i. the HOSA bank holds 138 questions (M14 Phase 2a-2d took word roots, prefixes, suffixes and anatomy to 30 each)");
  assert.equal(new Set(hosaBank.MEDTERM_BANK.map((q) => q.id)).size, 138, "26j. with unique ids");
