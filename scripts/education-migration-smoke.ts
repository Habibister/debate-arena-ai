...
function assertPhase1aResolverInvariants(file: string, label: string) {
  ...
  assert.equal(EDUCATION_LESSONS.length, 21, "5. twenty-one canonical registry entries");
  const learnerVisible = EDUCATION_LESSONS.filter((e) => e.visibility === "learner");
  assert.equal(learnerVisible.length, 21, "6a. all twenty-one are learner-visible (P1-B4 added the six marketing lessons)");
  ...
  assert.equal(educationLessonsForTrack("DECA").length, 10, "6b. ten DECA lessons across two courses");
  assert.deepEqual(educationLessonsForTrack("DECA").map((e) => e.id),
    ["how-deca-roleplay-works", "deca-understanding-performance-indicators", "deca-justifying-your-recommendation", "deca-handling-customer-situations",
    ...
    "6b2. orientation, the two role-play owners, the customer-relations owner, then MK1-MK6");
  ...
  assert.equal((getEducationLesson("debate-round-orientation") as { practiceDrill?: unknown } | undefined)?.practiceDrill, undefined,
    "25b. the Wave 1A orientation carries NO drill mapping — Taught-only, never remediation-mapped");
  ...
  // provably exists. These controls bind the mapping, its exactness, and its absence elsewhere.
  {
    ...
    // P1-B2 added the eighth: the DECA business-reasoning owner, directly after it in registry order.
    assert.deepEqual(mapped.map((e) => e.id),
      ["debate-evidence-evaluation", "debate-answer-types", "debate-turn-mechanics", "deca-understanding-performance-indicators", "deca-justifying-your-recommendation", "deca-handling-customer-situations", "deca-who-the-customer-is", "deca-why-they-choose-you", "deca-how-you-are-understood", "deca-the-offering-and-its-price", "deca-getting-it-to-the-customer", "deca-telling-them-about-it", "debate-clash", "debate-refutation", "debate-weighing"],
      `36. exactly fifteen authored lessons name a drill destination — evidence, answer-types (CTA-only), turn-mechanics (CTA-only), the two DECA role-play owners, the DECA cluster-knowledge owner, clash, refutation and weighing  [${mapped.map((e) => e.id).join(", ")}]`);
