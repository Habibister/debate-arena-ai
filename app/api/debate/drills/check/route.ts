...
// Record ONE answer to a served General Debate drill question and return its feedback (M13E2 C2).
//
...
//
// DELIBERATELY NOT RATE-LIMITED. The existing `light` tier is 20 requests/minute, and a twenty-slot
// drill legitimately needs one call per question plus a start and a submit — limiting it would break
// ordinary practice. The endpoint is safe without one: it requires authentication, is scoped to a
// session the caller owns, and each item accepts exactly one write, so the write surface is bounded
// by that session's own item count. Rate-limit redesign is deferred, as approved.
