...
function assertPhase1aResolverInvariants(file: string, label: string) {
  ...
  assert.equal(EDUCATION_LESSONS.length, 14, "5. fourteen canonical registry entries");
  const learnerVisible = EDUCATION_LESSONS.filter((e) => e.visibility === "learner");
  assert.equal(learnerVisible.length, 14, "6a. all fourteen are learner-visible (P1-B2 added the DECA business-reasoning lesson)");
  ...
  assert.equal(educationLessonsForTrack("DECA").length, 3, "6b. three DECA lessons");
  assert.deepEqual(educationLessonsForTrack("DECA").map((e) => e.id),
    ["how-deca-roleplay-works", "deca-understanding-performance-indicators", "deca-justifying-your-recommendation"],
    "6b2. orientation first, then the two role-play skill teaching owners in teaching order");
  ...
  // provably exists. These controls bind the mapping, its exactness, and its absence elsewhere.
  {
    ...
    assert.deepEqual(mapped.map((e) => e.id),
      ["debate-evidence-evaluation", "debate-answer-types", "debate-turn-mechanics", "deca-understanding-performance-indicators", "deca-justifying-your-recommendation", "debate-clash", "debate-refutation", "debate-weighing"],
      `36. exactly eight authored lessons name a drill destination — evidence, answer-types (CTA-only), turn-mechanics (CTA-only), the two DECA role-play owners, clash, refutation and weighing  [${mapped.map((e) => e.id).join(", ")}]`);
