  ...
  },
  {
    ...
    lesson: {
      ...
      content: lesson(
        ...
        ["In a speech that answers what the other side said, give a roadmap that names what the answers are on, in the order you will take them; a bare count does a weaker version of the same job.", "Name the specific argument before you answer it, using words that identify it rather than its number or its speaker.", "Moving to another of their arguments needs no separate transition — the next label does it; add one only for a larger move, such as returning to your own case.", "If you lose the thread, say where you are going next instead of pressing on \u2014 and if you drop an answer you promised, say so as you pass it."],
        {
          ...
          whyItWorks: "Delete the roadmap and the three labels from the strong version and what is left is the weak version, word for word — the same three claims, the same three reasons behind them, the same quality of analysis. Nothing was added to the argument. What the labels change is where each answer gets written down: the judge hears \"on cost\" and moves to the cost column before the reason arrives. The opening line does a second job the labels cannot: promising three answers gives the judge something to count against, so an answer that never arrives is visible as a gap rather than simply forgotten. Each of those three labels marks a move to a different argument, which is what earns it; a label in front of every sentence would earn nothing. Notice what the labels leave out — no reason, no verdict on the argument, no claim about how much of their case falls. \"On cost\" locates the answer and nothing more; everything that persuades comes after it. In the weak version the judge has to work out, after the fact, which of three unlabelled sentences belonged to which argument — and a sentence assigned to the wrong column is answering nothing. This is also why signposting cannot rescue a weak answer: it decides where the answer lands, not whether it was any good."
        },
        q(
        ...
        [
          q(
            ...
            ["I have three answers before I come back to our own case", "I will take their funding argument, then their staffing argument", "Two answers on their evidence, then I rebuild our access argument", "I am going to show that funding is what decides this round"],
          ...
          q(
            "You have been answering their arguments one after another. Which move earns a separate transition rather than just the next label?",
            ["Moving from their funding argument to their staffing argument", "Moving from one inner claim to another inside their funding argument", "Moving from answering their case to rebuilding your own access argument", "Moving from your second answer to your third on their evidence argument"],
            "Moving from answering their case to rebuilding your own access argument",
            "A label already closes one column and opens another. What can it not tell the judge?",
            "Leaving their arguments to rebuild your own changes the kind of work you are doing, and the next label alone does not say so. The other three stay inside answering their case, where the label you are already giving does the whole job.",
            "Signposting"
          ),
          q(
            "A speaker labels every answer clearly and gives no reason for any of them. What have the labels achieved?",
            ["Each answer is easier to follow, and being easy to follow is part of persuading a judge", "Each answer is recorded against the argument it was aimed at, and left unsupported", "Each answer is left unsupported, so the labels have achieved nothing at all", "Each answer reaches the judge, who places it once the speech has finished"],
            "Each answer is recorded against the argument it was aimed at, and left unsupported",
        ...
        [
        q(
          "They ran two arguments: that the fee deters families, and that the refund scheme is hard to use. Your answer is that the refund form takes two minutes. You label it \"on their argument that the fee deters families\". What is the result?",
          ["The answer is filed under deterrence, which it never touches, so the refund point keeps none", "The answer is filed against the refund scheme, since what it says makes the target obvious", "The label is too broad, because the answer is aimed at one claim rather than a whole argument", "The answer is wasted, because the minutes a refund form takes were never a reason about the fee"],
          "The answer is filed under deterrence, which it never touches, so the refund point keeps none",
          "The label was exact. Ask what it was exact about.",
          "A label can be perfectly precise and still name the wrong argument. The judge places answers as they arrive rather than reconstructing them afterwards, so the deterrence column now looks answered when it is not, and the refund argument keeps nothing at all. The answer itself was a good one \u2014 how long the form takes is exactly a reason the scheme is not hard to use \u2014 and the label is what wasted it. The level was right; the branch was wrong.",
          "Signposting"
        ),
        q(
          ...
          ["The judge hears the second name and files that answer under the first argument", "You lose the seconds it takes to explain that the two names mean the same thing", "The judge is left to work out for themselves that both names are one argument", "The roadmap you gave was inaccurate, and that is where the mismatch started"],
          "The judge is left to work out for themselves that both names are one argument",
          "The judge matches what you say against a column that already has a name.",
          "A label works by matching the argument the judge already has, so a second name for one thread hands the judge the matching to do. Nothing in the label itself tells the judge that the two names are one argument, and the cost is not the two seconds of explaining \u2014 it is that the matching is now theirs to do. The roadmap was accurate when you gave it; what drifted is the name, which is exactly what a consistent one would have held.",
          "Signposting"
        )
        ],
        {
  ...
  },
  {
    ...
    lesson: {
      ...
      content: lesson(
        ...
        ["Set out what your case will contain and in what order before you argue any of it.", "Order the contentions so that anything the later ones depend on comes first.", "Give each contention a job the others do not do, and develop it rather than mentioning it.", "Use evidence by saying what it found and why that supports the warrant, not by naming it.", "Close on what your case establishes, without claiming a comparison the round has not had yet.", "Before you finish, check that each contention does a distinct job in the case and has actually been developed rather than named."],
        {
          ...
          strongAnswer: "Two contentions: first that the requirement improves academic integrity, second that it improves career readiness. On integrity: students who are taught what these tools can and cannot do cite them instead of concealing them, and a district that ran the training saw fewer unattributed submissions the following year — which is what you would expect if concealment is driven by not knowing what counts as allowed. On career readiness: the same habits transfer, because entry-level work increasingly involves checking machine-generated output, and a student who has practised verifying a source is doing the thing the job asks for. Together those two show that teaching students what these tools do changes what they declare in school and what they can already do at work.",
          whyItWorks: "The weak version is not missing content — it is a rebuttal delivered in a constructive slot. It spends the speech answering arguments the other side has not made yet, so the judge hears the opposition's case first and your two contentions arrive as fragments: integrity gets a study with no finding attached, careers gets a sentence with no warrant. The strong version establishes the same two contentions instead of defending them prematurely, gives each one a job the other does not do, and uses the study by saying what it found and why that supports the warrant. Neither version compares itself to a case that has not been heard yet — that work belongs in later speeches."
        },
        q(
          "Which of these is a job the constructive does that a later speech cannot easily do for it?",
          ["Comparing your impacts against the other side's impacts once both are in the round", "Answering the strongest argument the opposition has actually made", "Deciding which single argument the judge should vote on at the end", "Putting your own contentions on the table so the round can run against them"],
          "Putting your own contentions on the table so the round can run against them",
          "Which of these has to happen first for the others to be possible?",
          "Establishing the case is the constructive's distinctive job — the other three all depend on material that is already in the round. Comparison, refutation and collapsing are later work, and each needs a case that a constructive has already built.",
          "Constructive"
        ),
        [
          q(
            "A speaker's constructive spends most of its time answering arguments the other side has not made yet. What is the main cost?",
            ["The speech will run over time, because pre-empting an argument always takes longer", "The judge is not permitted to consider pre-emptive arguments in a constructive", "The arguments become unusable later because they were raised too early", "The speech introduces the opponent's case while leaving its own underdeveloped"],
            "The speech introduces the opponent's case while leaving its own underdeveloped",
            "Ask what the judge ends up with at the end of the speech.",
            "Pre-empting spends your speech on their case and leaves yours thin. The cost is what the judge has at the end, not a timing problem or a rule against it — nothing forbids pre-empting, it just trades your own development for theirs.",
            "Constructive"
          ),
          q(
            "Which use of a source actually supports the contention it sits in?",
            ["A 2024 district study is directly relevant to what this contention argues", "Several published sources point the same way on this question", "A district that ran the training saw fewer unattributed submissions the next year", "This contention is supported by a study, which the other side has not disputed"],
            "A district that ran the training saw fewer unattributed submissions the next year",
            "One of these tells the judge what the source found.",
            "Only this states a finding the warrant can rest on. The others name a study, assert that evidence exists, or point out that it went unanswered — all of which tell the judge support has been claimed without telling them what it is.",
            "Evidence"
          ),
          q(
            "A case for keeping the town's library open on Sundays runs two contentions, each argued in full. One argues that Sunday opening is what lets people who work weekdays use the library at all. The other argues that a library closed on Sundays is one that the people whose working week leaves them no other day cannot use. What is the problem with the case?",
            ["Nothing is wrong, because a case is stronger when the same thing is established twice", "Both contentions establish the same thing, so the case has the look of two parts and the substance of one", "The two contentions are in the wrong order, because the one about who is shut out has to be argued first", "The case has not told the judge which of its two contentions the round should be decided on"],
            "Both contentions establish the same thing, so the case has the look of two parts and the substance of one",
            "Suppose the judge accepts both contentions, and ask how many things the case has then established.",
            "Both contentions establish one thing between them — that the people whose working week leaves them no other day can use the library only if it opens on a Sunday — so a judge who accepts either has accepted the other on the way, and the case has the look of breadth without a second part to it. Establishing one thing twice does not make it more established; it spends a stretch of speech that a genuinely different argument would have had. No order is forced here either, because neither contention needs anything the other supplies. And choosing what the round should turn on is later work, not what is missing from a case that has argued one thing twice.",
            "Speech organization"
          )
        ],
        [
          q(
            ...
            ["Open by answering the two arguments the other side used in their previous round", "Order the contentions by what depends on what, develop each, then say what they establish", "Present every argument your side has, briefly, so that nothing is left out of the round", "Lead with the comparison you want the judge to make at the very end of the debate"],
            "Order the contentions by what depends on what, develop each, then say what they establish",
            "Match the plan to the speech's job: establishing your case.",
            "This plan establishes the case, orders it around real dependencies, gives each contention development, and closes on what was shown. Answering last round's arguments responds to nothing in this one, listing everything trades depth for coverage, and leading with the final comparison asserts a conclusion the round has not earned.",
            "Constructive"
          ),
          q(
            "A speaker delivers two well-warranted contentions and sits down after the second one's impact. What has the speech left undone?",
            ["Nothing, because two well-warranted contentions are a complete constructive", "It has not told the judge what the contentions add up to for the case", "It has failed to provide the evidence that both contentions required", "It has not pre-empted the responses the other side is most likely to give"],
            "It has not told the judge what the contentions add up to for the case",
            "Two good arguments are not automatically one case.",
            "Contentions support a case, and the speech should say what they establish together — otherwise the judge has two arguments and has to assemble the case themselves. Two well-warranted contentions are not already a complete constructive for exactly that reason. The warrants were fine, evidence is a separate question, and pre-empting is not the constructive's job.",
            "Constructive"
          )
        ],
        {
          revisionLadder: [
            { attempt: "My case for free swimming lessons has two contentions. DROWNING: a child who has been taught can get themselves out of trouble in the water, so fewer children drown. ACCESS: the families who cannot afford the pool are the ones whose children never learn, so taking the charge away reaches the children who are missing out now. I will open on drowning, because it is the strongest thing I have.", diagnosis: "Both contentions are argued, and the plan still cannot be run in this order. The drowning contention is about children who cannot currently swim; access is where the speech argues who those children are and why the charge is what keeps them out of the water. Opening on drowning asks the judge to accept it while the thing it stands on is still to come. Strength is a real consideration, but only where nothing forces the sequence — and here something does. A contention that another one rests on does not become background because it is the less dramatic of the two.", revision: "Access first: the charge is what keeps particular children out of the pool, and here is who they are. Then drowning: those are the children the drownings fall among, so teaching them is what changes the number. Nothing in the second contention now arrives before the thing it stands on." },
            { attempt: "Access, then drowning. I have room for a third contention and I want to add one: that a child who has been taught is at ease in deep water instead of frightened of it. It can go last — nothing in the case depends on it, so the order still holds.", diagnosis: "The reasoning about order is right, and the new contention does not disturb it: where nothing rests on a contention, nothing forces where it sits, and it can go wherever the speech has room for it. What it does not have is a job of its own. Being at ease in deep water instead of panicking is part of why a taught child gets out of trouble, which is what the drowning contention already argues — a judge who accepts that one has accepted this on the way. Two contentions doing substantially the same job give the case the look of breadth without a second part to it, and speech attention is finite, so the stretch is not free: it comes out of the two contentions that do stand apart.", revision: "Keep access and drowning, and let the material about being at ease sit inside the drowning contention, where it does its work. Give the spare stretch to something the case is not yet arguing at all — that the pool sits unused during school hours, so the lessons take time the town is already paying for. That does a job neither of the others does, and since nothing in the case rests on it, it can sit wherever there is room." },
            { attempt: "Access, then drowning, then the unused hours, each argued in full. I finish: \"Those are our three contentions, and taken together they are worth more than anything the other side is going to put up against them.\"", diagnosis: "The plan is sound and the last sentence is the only thing left to repair. It measures this case against one nobody has heard, which the speech has no basis for; and while it is doing that, it never says what the three contentions establish, so the judge is left to assemble the case out of the parts. Two faults sit in one sentence, and one replacement clears both — nothing else in the plan needs touching.", revision: "…closing instead: \"Together those three show that the charge is what keeps particular children out of the water, that those are the children the drownings fall among, and that the town can teach them in hours it is already paying for.\" That is what the case establishes, and it claims nothing about a case that has not been made." }
          ],
          scaffoldedTry: {
            prompt: "A case for opening the school sports halls to the public in the evenings has three contentions, and all three are already argued in full — none of the arguing here is yours. They are set out below in the order the speaker happened to draft them, which is not necessarily an order the speech can be argued in.\n\nCLUBS: the five local clubs that currently drive to the next town would move their sessions here, and they would move because the hire fee is low enough to make the move worth it — which it is only if opening the halls costs the school nothing it is not already spending.\n\nHOURS: the halls stand empty after the school day and the caretaking contract already pays for the building into the evening, so the school can open them without spending anything extra.\n\nUPKEEP: the hire fees the evening sessions bring in clear the halls' repair backlog within three years.\n\nPut the three in an order the speech can be argued in, say which contention supplies the fees UPKEEP counts, and write the one sentence you would close on.",
            frame: "Order: ___. The fees UPKEEP counts are paid by the people argued for in ___. I would close: ___",
